...
// // '4.5'
// --- - ---
export const getBalance1 = async (provider, address) => {
  try {
    const balance = await provider.getBalance(address);
    console.log(' Balance -> ', balance, ' - in Func ');
    return balance;
  } catch (err) {
...
export const getContractBalance = async (contract, address) => {
  try {
    const onContractBalance = await contract.balanceOf(address);

    console.log(
      ' -- onContractBalance: ',
      onContractBalance,
      ', -> ',
      ethers.formatEther(onContractBalance),
    );
    return onContractBalance;
  } catch (err) {
    console.error(' ERROR: ', err);
  }
};
